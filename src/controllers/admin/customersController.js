...
export async function getCustomers(req, res, next) {
  try {
    ...
    const showingFrom = total ? (safePage - 1) * limit + 1 : 0;
    const showingTo   = Math.min(safePage * limit, total);

    if (isAjax) {
      return res.json({ users, total, totalPages, currentPage: safePage, showingFrom, showingTo, q, sort });
    }
    ...
    return res.render("admin/auth/customers", {
      users, currentPage: safePage, totalPages, total, limit, query: q, sort, showingFrom, showingTo,
    });
...
export async function deleteUser(req, res, next) {
  try {
    ...
    if (user.isAdmin) throw new AppError(403, "Admin users cannot be deleted");
