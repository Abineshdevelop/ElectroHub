...
function validateOfferBody({ offerName, offerType, refId, offerPercentage, startDate, endDate }) {
  const name = offerName?.trim();
  if (!name) return "Offer name is required.";
  if (name.length < 3) return "Offer name must be at least 3 characters.";
  if (name.length > 30) return "Offer name must not exceed 30 characters.";

  if (!offerType) return "Offer type is required.";
  if (!["product", "category"].includes(offerType)) return "Offer type must be 'product' or 'category'.";

  if (!refId || !String(refId).trim()) return "Please select a valid product or category.";
  ...
  if (isNaN(numericPercentage)) return "Offer percentage must be a number.";
  if (numericPercentage < 1) return "Offer percentage must be at least 1%.";
  if (numericPercentage > 100) return "Offer percentage cannot exceed 100%.";
  if (!Number.isInteger(numericPercentage)) return "Offer percentage must be a whole number.";

  if (!startDate) return "Start date is required.";
  if (isNaN(new Date(startDate).getTime())) return "Start date is invalid.";

  if (!endDate) return "Expiry date is required.";
  if (isNaN(new Date(endDate).getTime())) return "Expiry date is invalid.";
  if (startDate && new Date(endDate) <= new Date(startDate)) return "Expiry date must be after the start date.";
...
async function populateOfferRef(offer) {
  ...
  try {
    if (plainOffer.offerType === "product") {
      const product = await Product.findById(plainOffer.refId).select("productName").lean();
      plainOffer.refId = product ? { _id: product._id, productName: product.productName } : null;
    } else if (plainOffer.offerType === "category") {
      const category = await Category.findById(plainOffer.refId).select("categoryName").lean();
      plainOffer.refId = category ? { _id: category._id, categoryName: category.categoryName } : null;
    }
  } catch {
...
async function applyOfferToCategory(categoryId, percentage) {
  const products = await Product.find({ categoryId, isActive: true, isDeleted: false });
...
export const getOffersPage = async (req, res) => {
  try {
    ...
    const showingTo   = Math.min(skip + PER_PAGE, totalOffers);
    ...
    if (isAjax) return res.json({ success: true, ...responseData });
...
export const getOfferById = async (req, res) => {
  try {
    ...
    const rawOffer = await Offer.findOne({ _id: req.params.id, isDeleted: false }).lean();
    if (!rawOffer) return res.status(404).json({ success: false, message: "Offer not found." });
...
export const searchProducts = async (req, res) => {
  try {
    ...
    if (searchQuery.length > 100) return res.status(400).json({ results: [], message: "Query too long." });
    ...
    res.json({ results: products.map((p) => ({ _id: p._id, name: p.productName })) });
  } catch (err) {
    console.error(err);
...
export const searchCategories = async (req, res) => {
  try {
    ...
    res.json({ results: categories.map((c) => ({ _id: c._id, name: c.categoryName })) });
  } catch (err) {
    console.error(err);
    res.status(500).json({ results: [] });
  }
...
export const createOffer = async (req, res) => {
  try {
    ...
    const validationError = validateOfferBody({
      offerName,
      offerType,
      refId,
    ...
    if (offerType === "product") {
      const exists = await Product.findOne({ _id: refId, isActive: true, isDeleted: false });
      if (!exists) return res.status(400).json({ success: false, message: "Selected product not found or inactive." });
    } else {
      const exists = await Category.findOne({ _id: refId, isDeleted: false });
      if (!exists) return res.status(400).json({ success: false, message: "Selected category not found." });
    }
    ...
    console.error(err);
    if (err.name === "CastError") return res.status(400).json({ success: false, message: "Invalid product or category ID." });
...
export const editOffer = async (req, res) => {
  try {
    ...
    if (duplicateOffer)
      return res.status(409).json({ success: false, message: `Another active offer already exists for this ${offerType}.` });

    if (offer.isActive) {
      if (offer.offerType === "product") await removeOfferFromProduct(offer.refId);
      if (offer.offerType === "category") await removeOfferFromCategory(offer.refId);
    }
    ...
    console.error(err);
    if (err.name === "CastError") return res.status(400).json({ success: false, message: "Invalid ID provided." });
...
export const toggleOffer = async (req, res) => {
  try {
    ...
    if (offer.isActive) {
      if (offer.offerType === "product") await applyOfferToProduct(offer.refId, offer.offerPrecentage);
      if (offer.offerType === "category") await applyOfferToCategory(offer.refId, offer.offerPrecentage);
    } else {
      if (offer.offerType === "product") await removeOfferFromProduct(offer.refId);
      if (offer.offerType === "category") await removeOfferFromCategory(offer.refId);
    }
